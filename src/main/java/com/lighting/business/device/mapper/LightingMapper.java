...
public interface LightingMapper extends BaseMapper<Lighting> {
	...
	@Select("select t.is_online As isOnline,t.id As sponId,t.tname As sponName,t.tbcoutv,n.device_id As brightDeviceId,b.relieved,v.DZBH,v.status As evseStatus,v.DZMC,v.GZZT,l.*,a.device_name  as adscreenName,a.card_number as cardNumber,c.device_alias  as  deviceAliasc,b.device_alias as alarmboxName,d.device_name as sensorName,l.LIGHTINGNAME,	n.ON_OFF  as  onOff,n.BRIGHTNESS,n.NODE_ID,n.ALIAS,n.`STATUS` from t_lighting l "
			...
			+ "left join (select * from t_terminal) t on l.SPONID=t.id  "
			+ "${ew.customSqlSegment}"
	...
	@Select("SELECT  n.*,n.camera_uuid As cameraUuid,l.latitude As lightingLatitude,l.longitude As lightinglongitude,l.address, l.LIGHTINGNAME  FROM  (select * from t_lighting  WHERE ISDELETED = 0) l  RIGHT JOIN camera  n ON n.id = l.CAMERAID  "
			+ "${ew.customSqlSegment}")
	...
	@Select("SELECT	*  FROM 	t_lighting RIGHT JOIN t_terminal ON SPONID =id "
			+"${ew.customSqlSegment}"
			+" and LIGHTINGID IS NULL  OR LIGHTINGID = '' and isdeleted = '0'")
	List<LightingWithSpon> getSponNotBind(@Param("ew") QueryWrapper<Lighting> wrapper);

	@Select("select " +
			"(select count(1) from t_bright where isdeleted = 0 and projectid = '${projectid}' and areaid = '${areaid}' and projectid in (${projectids}) and areaid in (${areaids})) as brightsNum, " +
			"(SELECT COUNT(1) from t_ad_screen_device where deleted = 0 and project_id = '${projectid}'  and area_id ='${areaid}'  and project_id in (${projectids}) and area_id in (${areaids})) As adsNum, " +
			"(select COUNT(1) from t_lighting WHERE isdeleted = 0 and projectid = '${projectid}'  and areaid = '${areaid}'   and projectid in (${projectids}) and areaid in (${areaids})) As lightingsNum, " +
			"(SELECT COUNT(1) from camera where deleted = 0 and project_id = '${projectid}' and area_id ='${areaid}' and state <> 3  and project_id in (${projectids}) and area_id in (${areaids})) As camerasNum, " +
			"(SELECT COUNT(1) from alarm_box where deleted = 0 and project_id = '${projectid}'  and area_id ='${areaid}'  and state <> 3  and project_id in (${projectids}) and area_id in (${areaids})) As alarmBoxsNum, " +
			"(SELECT COUNT(1) from t_evse_device where deleted = 0 and project_id = '${projectid}' and area_id ='${areaid}'  and project_id in (${projectids}) and area_id in (${areaids})) As evsesNum, " +
			"(SELECT COUNT(1) from t_envir_devices where deleted = 0 and project_id = '${projectid}' and area_id ='${areaid}'  and project_id in (${projectids}) and area_id in (${areaids})) As sensorsNum, " +
